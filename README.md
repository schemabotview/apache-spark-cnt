...
scripts (`tts/` → `audio/` `.wav` via Colab), and the wiring `manifest.json`. Nothing to
build or run here. For the authoring contract and folder layout, see
[`CLAUDE.md`](./CLAUDE.md).

**This file is the course outline** — the human-facing map of modules and sections. It is the
plan we author against; the machine source of truth for structure is `manifest.json`.

**Status:** scaffolding + spine categorization (this file). No sections authored yet. Modules
and sections below are the **proposed spine — under review**; headings settle per module as
authored.
...
Concept → **modules** (~10) → **sections** (~10). **A section is the atomic unit** the video
steps through. Every artifact for a section shares the stem `NN-SS-slug` (`NN` = module
number, `SS` = section position — a sorted glob stays in reading order): `md/NN-SS-slug.md`
(source of truth) + `slides/NN-SS-slug.slide` + `tts/NN-SS-slug.tts` → `audio/NN-SS-slug.wav`,
all wired by `manifest.json`.

---

## Module spine (proposed — 10 modules × ~10 sections)

Shaped into the video target: **10 modules × ~10 tight teaching sections**, each a narrated
slide + scene focus (≈ one page). Silent "What's covered" / self-check / "Summary"
scaffolding is dropped; each module's §01 is a real teaching **hook**.

Scene assignment: **01–02 → `spark-architecture`**, **08 → `spark-streaming`**, **all others
→ `spark-batch-api`** (the shared batch / DataFrame / RDD / SQL / perf master map). Scenes live
in the UI repo (app-owned TypeScript), referenced from the manifest by id.
...
Parked (not in the video set): a sample-exam-questions module and a hands-on capstone
project — Q&A / open-ended is a weak fit for the narrated-scene format.

---

## Sections (proposed spine — final headings settle per module as authored)
...
> Note: AQE appears twice by design — a light "what it is" beat in 02 (foundation) and the
> deep "runtime re-optimization" in 09 (tuning).

---

## Reference material

- `../../Products/apache-spark-ct/` — the prior video-target Spark repo (`.ipynb`-based).
  **Structure reference** for this `.md`-based rebuild; narration is re-authored fresh and the
  owner regenerates the `.wav`s via Colab (old `.wav`s are never reused).
- `../../Products/apache-spark-content/` — the graphl-ux-era Spark curriculum (per-module
  notebooks + outline). **Prose reference only.**
